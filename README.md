...
- [x] 強化学習(`src/codebot/rl`) — GRPO(Group Relative Policy Optimization)、報酬はコード実行結果(テスト通過+短いほど高得点)
  ...
  - `data/rl/tasks.jsonl` — テストアサーション付きのコードゴルフお題6件
  - `scripts/rl.py` — SFTチェックポイントを起点に(同じ重みを凍結した参照モデルとしても使用)GRPOで学習、`data/checkpoint_rl.pt`に保存
  - 「アドバンテージが正の時に実際にそのレスポンスの確率が上がる」ことをユニットテストで直接確認済み。ただし現状のモデルは学習量が少なすぎてどのタスクも一度もテストに通らず、実データでの通しデモでは全ステップがreward=0でスキップされる(仕組みは動くが、報酬シグナルが立つにはSFTモデルの実力がもう少し要る)
